...
# Function to generate the identifier of the flavour of this RSA analysis
# i.e. which brain mask, distance methods, rating type
generate_results_flavour <- function(gm_file, dist_method_rating, 
                                     dist_method_fmri, dist_method_rsa, subs) {
...
# Function to write RSA values to a nii
# The input df_RSA must have three columns:
# - sub <chr> : e.g. "02"
# - region <int> : idx to the voxel in the 91x109x91 volume
# - similarity <dbl> : the actual RSA value
#
# These are derived from all_similarities using
write_RSA_results_to_nii <- function(df_RSA, ratings_type, results_dir, gm) {
  
  results_nii = gm
  
  idx_voxel <- df_RSA$region
  
  # write R2Z if the distance method for rsa is the Pearson R
  if(dist_method_rsa == "pearson") {
    results_nii[idx_voxel] = atanh(df_RSA$similarity) # Fisher r to Z 
  } else {
    results_nii[idx_voxel] = df_RSA$similarity
  }
  
  # In the unlikely event of Infs (atanh(r) where r = 1 or -1) or NAs
  # replace those with zeros
  results_nii[!is.finite(results_nii)] = 0
  
  sub_ID <- df_RSA$sub[1]
  
  nii2write_path <- paste0(
    results_dir,"/", "sub_", sub_ID,"_RSA_",ratings_type,".nii.gz"
  )
