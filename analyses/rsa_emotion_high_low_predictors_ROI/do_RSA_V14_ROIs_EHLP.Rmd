...
Here we include two modifications of the RSA calculation:

- selecting only the most discriminatory elements in RDM_rats using a RM-ANOVA or three paired ttests

- using the residuals of each rdm_[type] with respect to the other two, e.g. 
residuals of rdm_emotion wrt rdm_aroval and rdm_arousal (using a GLM)

NOTE THAT ALL OF THIS INVOLVES ONLY RDM_rats, WE ARE NOT DEALING WITH THE RAW RATINGS.

It is possible to switch between these options in the code below (around lines 550-555):
```
# Use the following to use the residuals instead of the original RDM_rats 
# Replace the RDMs_rats with RDMs_rats_parcorr
RDMs_rats <- RDMs_rats_parcorr

# Use only one of the following to carry out the selection of RDM_rats elements either on RM-ANOVA or three paired ttests

# Uncomment below to use the sig from RM-ANOVA
idx_sig <- which(RDMs_RM_ANOVA_results$p < p_thresh)

# # Uncomment below to use the sig from 3 paired ttest
# idx_sig <- which(RDMs_RM_ttest_results$sig == 1)
```
...
# Calcule the residuals of each rdm_[emotion/arousal/aroval] after regressing out the other two.
# We wil use this later in the calculation of the RSA
```{r}
...
rdm_pairwise_corrs <- function(RDMs_rats, emo = "rdm_emotion", aro = "rdm_arousal", aroval = "rdm_aroval") {
  RDMs_rats %>%
    mutate(
...
# RM-ANOVA and three-ttests on each element of the RDM_rats to identify the locations
# where there is a sig difference
```{r}
...
# DANGER AHEAD: 
MODIFICATION TO INCLUDE ONLY THE SIG ELEMENTS OF THE RDM
AND USE RESIDUALS OF THE RDM FOR THE RSA.

We now want to run the RSA only on the sig elements of the RDM and using
the parcorr versions of the RDM. 

In order not to create too much mess, we will simply 
1. filter the elements in RDMs_fmri and RDMs_rats
2. replace `RDMs_rats` with `RDMs_rats_parcorr`

So **THE ORIGINAL `RDMs_fmri` AND `RDMs_rats` WILL BE OVERWRITTEN!!!**
...
# retain for each rdm for each sub only the elements which are in idx_sig[ttest/RM_ANOVA]
# both for RDMs_fmri (colum rdm_fmri) and RDMs_rats (columns rdm_emotion, rdm_arousal, rdm_aroval)
RDMs_rats <- RDMs_rats %>%
  mutate(
...
create_filename_results <- function(atlas_filename, copes_type, remove_neutrals) {
  ...
  results_filename <- paste0(atlas_root, EV_abbr, option_MN, option_NN, nsubs, "___sig_parcorr")
