...
# regress_motion_predictors
The aim is to regress motion energy from the ratings RDM before calculating the 
RSA. Since the order of stimuli is different for each participant, the "easiest"
...
```bash
# Calculates the motion energy
/data00/leonardo/RSA/prep_scripts/07_motion_energy_movies/do_motion_energy.ipynb

# Produces an output which is compatible with the other <model_type>_ratings.csv
...
Or course the motion energy estimates are _not_ ratings, however after 1,000,000
requests of "Why don't we do this and that...? It's easy!" I **had** to give up order
and create some mess. So here it is. If you don't like it, go somewhere else.
...
create_filename_results <- function(atlas_filename, copes_type, remove_neutrals) {
  ...
  results_filename <- paste0(atlas_root, "_reg_ME_", EV_abbr, option_MN, option_NN, nsubs)
