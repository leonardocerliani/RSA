...
congruence %>%
  ...
  geom_line(data = median_corrs, aes(x = sub, y = median_corr, group = 1), color = "red", size = 0.8) +  # Corrected geom_line
  labs(
    title = "Correlation of each Subject Ratings with Mean across Subjects  (Leave-One-Out)",
...
congruence_initial %>%
  ...
  labs(
    ...
    subtitle = "cutoff at 0.84",
    x = "Subject ID"
  ) +
  # geom_hline(yintercept = congruence_initial$ALL %>% mean, linetype = "dashed", color = "blue")
  geom_hline(yintercept = 0.84, linetype = "dashed", color = "blue", size = 0.8)
...
congruence_initial %>%
  ...
  labs(
    title = "Congruence between displayed emotion and max rated emotion",
    # subtitle = paste0("mean : ", congruence_initial$ALL %>% mean %>% round(2)),
    subtitle = "cutoff at 0.82",
    x = "Subject ID"
  ) +
  # geom_hline(yintercept = congruence_initial$ALL %>% mean, linetype = "dashed", color = "blue")
  geom_hline(yintercept = 0.85, linetype = "dashed", color = "blue", size = 0.8)
```
...
congruence_initial %>%
  ...
  theme_minimal() +
  labs(
    title = "Congruence between displayed and max rated emotion as % of total movies (56)",
    # subtitle = paste0("mean : ", congruence_initial$ALL %>% mean %>% round(2)),
    subtitle = ("The cutoff is the percentage of correctly identified (i.e congruently highest rated)
movies on a large cohort of subjects who carried out only the rating task"),
    x = "Subject ID"
  ) +
  # geom_hline(yintercept = congruence_initial$ALL %>% mean, linetype = "dashed", color = "blue")
  geom_hline(yintercept = 0.84, linetype = "dashed", color = "blue", size = 0.8)
...
df %>%
  ...
  labs(
    ...
    scale_x_discrete(labels = function(x) str_remove(x, "^r_"))  # Remove 'r_' prefix from x-axis labels
...
# Define the median values
# median_cong_OLD <- congruence_initial$ALL %>% mean
cutoff_cong_OLD <- 0.84
median_cong_NEW <- median_corrs$median_corr %>% median
...
congruence_initial %>% 
  ...
  geom_text(aes(label = sub), vjust = 0, hjust = 0) +  # Text labels
  # Add dotted vertical line at median_cong_OLD
  geom_vline(xintercept = cutoff_cong_OLD, linetype = "dashed", color = "red") +
  # Add dotted horizontal line at median_cong_NEW
  geom_hline(yintercept = median_cong_NEW, linetype = "dashed", color = "blue") +
  theme_minimal() +
  labs(
    ...
    title = "Two measures of congruence: % congruent movies within subject vs LOO correlation", 
    subtitle="blue line : median; red line: cutoff from validation study"
  )
