...
# # NB : in the LM version we use all ratings together in a model, so the 
# # ratings_type is set later
...
rsa_results_path <- paste0(bd, "/rsa_LM/rsa_results")
...
do_calc_RDM <- function(ratings_type) {
  ...
  return(RDMs_rats %>% group_by(sub) %>% mutate(triu_element = row_number()))
  
}


RDM_emotion <- do_calc_RDM("emotion") %>% rename("emotion" = value)
RDM_arousal <- do_calc_RDM("arousal") %>% rename("arousal" = value)
RDM_valence <- do_calc_RDM("valence") %>% rename("valence" = value)

RDMs_rats <- RDM_emotion %>% 
  inner_join(RDM_arousal, by=c("sub","triu_element")) %>% 
  inner_join(RDM_valence, by=c("sub","triu_element")) %>%
  select(!triu_element) %>% 
  group_by(sub) %>% 
  nest
...
# in the LM flavour we have already previously nested by sub
...
uff <- ff %>% 
  filter(sub == "02", region == "304150") %>% 
  unnest
  mutate(across(c(value,emotion, arousal, valence), ~ scale(.)))


fit <- lm(value ~ emotion + arousal + valence, data = uff)
...
THE FOLLOWING WAS THE ORIGINAL RSA
```{r}
