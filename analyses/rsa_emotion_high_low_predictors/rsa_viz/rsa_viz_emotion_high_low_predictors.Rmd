...
plot_heatmap_sides <- function(M, title=NULL) {
  ...
  paletta_emotions <- c(
    "#eb1414", "#f7a1a1",  # anger_high, anger_low
    "#0066ff", "#99c2ff",  # pain_high, pain_low
    "#00cc00", "#99ff99",  # disgust_high, disgust_low
    "#ff00ff", "#ff99ff",  # sad_high, sad_low
    "#ff5500", "#ffbb99",  # fear_high, fear_low
    "#ffff00", "#ffff99",  # happy_high, happy_low
    "#000000", "#cccccc"   # neutral_high, neutral_low
  )
  
  # Define the extended labels for emotions with "_high" and "_low" variations
  emo_labels <- c(
    "anger_high", "anger_low", 
    "pain_high", "pain_low", 
    "disgust_high", "disgust_low", 
    "sad_high", "sad_low", 
    "fear_high", "fear_low", 
    "happy_high", "happy_low", 
  ...
  heatmap.2(M, Rowv = NA, Colv = NA, symm=T, revC = F, col = blue_to_red,
            ...
            main = title, trace='none', cellnote=round(M, 2), notecol="black", notecex=0.03, 
...
# in get_rats_median_RDM : ratings_type is one of arousal, emotion, valence
...
get_rats_median_RDM <- function(ratings_type, dist_method_rating) {
  ...
  # EMOTION_HIGH_LOW_PREDICTORS VARIANT
  rats <- rats %>%
    filter(sub %in% subs) %>%
    # EMOTION COL HERE *MUST* ALSO SPECIFY HIGH AND LOW
    mutate(HI_LO = str_extract(high_low_code, "high|low")) %>%
    mutate(emotion = paste0(emotion,"_",HI_LO)) %>% 
    select(sub, emotion, starts_with("r_")) %>% 
    # IMPORTANT : AVG ACROSS EMOTION *AND* HIGH/LOW (12 + 2 predictors)
