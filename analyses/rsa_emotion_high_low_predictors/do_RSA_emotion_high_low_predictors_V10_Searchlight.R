...
source("funs_V10_emotion_high_low_predictors_Searchlight.R")
...
copes_type <- "emotion_high_low_predictors"
...
rsa_results_path <- paste0(bd, "/rsa_emotion_high_low_predictors/rsa_results")
...
# ------- IMPORTANT : AVG ACROSS MOVIES FOR [EMOTION]_HIGH AND [EMOTION]_LOW PREDICTORS MODEL -----
rats <- rats %>% 
  select(sub, emotion, starts_with("r_"), high_low_code) %>% 
  mutate(high_low_code = str_match(high_low_code, "high|low")) %>% 
  mutate(emotion = paste0(emotion,"_",high_low_code)) %>% 
