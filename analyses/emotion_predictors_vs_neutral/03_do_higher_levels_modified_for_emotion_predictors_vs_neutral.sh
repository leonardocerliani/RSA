...
export model="emotion_predictors_vs_neutral"
...
# There are 7 copes in the emotion_predictors_vs_neutral model:
# 1. anger
# 2. disgust
# 3. fear
# 4. happy
...
fsf_grouplevel_template="${analyses_dir}/grouplevel_template_modified_for_emotion_predictors_vs_neutral.fsf"
...
# run all the grouplevel copes (7 for emotion_predictors_vs_neutral) at once
grouplevel_fsfs=("$PWD/results/grouplevel_${model}_cope"{1..7}".fsf")
printf "%s\n" "${grouplevel_fsfs[@]}" | xargs -P 7 -I{} feat {}
