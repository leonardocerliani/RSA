...
do_RDM_ratings_one_sub <- function(sub_id, dist_method_rating, rats) {
  
  # filter the ratings for the required sub_id
  rats_one_sub <- rats %>% 
    filter(sub == sub_id) %>% 
  ...
  # extract RDM as tril of the D matrix
  D_feature_vector <- D_rats[!is.na(D_rats)]

  # return it as a column with the sub_id as header
  # NB: this fn will be used in a map_dfc
