...
OLE_plot_tril <- function(tril, model, n_copes = length(copes_numba), 
    labels_from = metadata_copes, reord = TRUE,
    fontsize = 6, side_mm=100, remove_neutrals) {
  ...
  # remove neutral copes and assign labels
  if (remove_neutrals == "YES") {
    labels <- metadata_copes %>% filter(emotion != "neutral") %>% select(label) %>% pull
  } else {
    labels <- metadata_copes$label
  }
  
  rownames(full_matrix) <- labels
  colnames(full_matrix) <- labels
