    ...
    mainPanel(width = 9,
              fluidRow(
                ...
                column(4,
                       ...
                       tags$h4("Aroval"), papayaOutput("papaya_valence")
                ),
...
do_boxplot <- function(RSA, roi_numba, ttest_type) {
  ...
  RSA %>% 
    ...
    select(!rsa_rdm_aroval) %>%   # REMOVING AROVAL (changed from valence)
    ...
    mutate(
      ...
      model = factor(model, levels = c("emotion", "arousal", "valence")) # Changed order to remove aroval and include valence
    ) %>% 
    ggwithinstats(
      x = model, y = value,
...
do_boxplot_4_download <- function(RSA, roi_numba, ttest_type) {
  ...
  ttest_results <- do_ttest_table(RSA, roi_numba) %>% 
    select(-aroval)  # Changed from valence to aroval
  print(ttest_results)
  ...
  RSA %>%
    ...
    mutate(
      model = str_replace(model, "rsa_rdm_",""),
      model = factor(model, levels = c("emotion", "arousal", "valence")) # Changed order
    ) %>%
    ggwithinstats(
      x = model, y = value,
      type = ttest_type,   # p, np, r, b
    ...
    theme(
      ...
      axis.text.y = element_text(size = 28),  # Y-axis text size remains 28
      axis.text.x = element_text(size = 18),  # X-axis text size changed to 18
