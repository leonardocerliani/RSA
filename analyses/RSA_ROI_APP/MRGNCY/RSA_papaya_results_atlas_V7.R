...
server <- function(input, output, session) {
  ...
  observe({
    ...
    output$papaya_model_selector <- renderUI({
      ...
      selectInput("model_choice", "", choices = model_choices, selected = "emotion")
    })
  ...
  observe({
    ...
    # output the table under the rois x mean_RSA table 
    output$ttest_table <- renderReactable({
      ...
      reactable(
        do_ttest_table(RSA, idx_ROI(), models_to_ttest),
        selection = "single",
  ...
  observe({
    ...
    # Motion_energy
    output$papaya_motion_energy <- renderPapaya({
      papaya_display("motion_energy", atlas_filename, input$positive_range[1], input$positive_range[2], input$negative_range[2], input$negative_range[1])
    })
    
    # Dynamic selection output (based on Dropdown menu)
    output$dynamic_papaya <- renderUI({
      req(input$model_choice)
      tagList(
        papayaOutput(paste0("papaya_", input$model_choice))
      )
    })
...
do_boxplot_4_download <- function(RSA, roi_numba, ttest_type, models_for_boxplot, MCP_correction_method) {
