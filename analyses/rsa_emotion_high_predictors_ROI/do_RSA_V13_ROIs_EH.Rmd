...
- param subs_set to choose between N14 and N26
...
rsa_flavour="rsa_emotion_high_predictors_ROI"
...
# ---------- SPECIFIC TO EMOTION HIGH ONLY -------------
# remove the [emotion]_low
df_path_copes <- df_path_copes %>% 
  filter(str_detect(emotion, "_high"))
# ---------- SPECIFIC TO EMOTION HIGH ONLY -------------
...
do_RDMs_rats <- function(ratings_type, remove_neutrals) {
  ...
  rats <- rats %>% 
    ...
    # ------ specific to emotion HIGH only predictors -----------
    filter(str_detect(emotion, "_high")) %>% 
...
create_filename_results <- function(atlas_filename, copes_type, remove_neutrals) {
  ...
  # handle the case of EH, since they are derived from EHLP
  if (str_detect(copes_type,"emotion_high_low_predictors")) {
    if (rsa_flavour == "rsa_emotion_high_predictors_ROI") {
      EV_abbr <- "_EH"
    } else {
      EV_abbr <- "_EHLP"
    }
