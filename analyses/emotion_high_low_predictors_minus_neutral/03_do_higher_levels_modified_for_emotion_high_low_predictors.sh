...
export model="emotion_high_low_predictors_minus_neutral"
...
# There are 14 copes in the emotion_high_low_predictors model:
# 1-2 anger      high/low
# 3-4 disgust    high/low
# 5-6 fear       high/low
# 7-8 happy      high/low
# 9-10 neutral   high/low
# 11-12 pain     high/low
# 13-14 sad      high/low
numba_copes=14

fsf_grouplevel_template="${analyses_dir}/grouplevel_template_modified_for_emotion_high_low_predictors.fsf"

for i in $(seq ${numba_copes}); do
...
# run all the grouplevel copes (14 for emotion_high_low_predictors) at once
grouplevel_fsfs=("$PWD/results/grouplevel_${model}_cope"{1..14}".fsf")
printf "%s\n" "${grouplevel_fsfs[@]}" | xargs -P 14 -I{} feat {}
