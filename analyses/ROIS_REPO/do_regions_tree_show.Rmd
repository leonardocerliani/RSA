...
# NB: to have colors, use collapsibleTreeSummary
# with the `attribute` parameter. See last cell below

htmlwidgets::saveWidget(tree, paste0("collapsibleTree_Yeo",Yeo7_ROI,".html"), selfcontained = TRUE)
...
df %>%
  collapsibleTree(
    root = NULL,
    hierarchy = c("Yeo7","Yeo17","HO_cort","anatomy_toolbox"), 
    collapsed = FALSE,
    fill = c(
      rep("brown", length(unique(df$Yeo7))),
      rep("khaki", length(unique(paste(df$Yeo7, df$Yeo17)))),
      rep("blue", length(unique(paste(df$Yeo7, df$Yeo17, df$HO_cort)))),
      rep("orange", length(unique(paste(df$Yeo7, df$Yeo17, df$HO_cort, df$anatomy_toolbox))))
    )
  )
...
org$tooltip <- paste0(
  ...
  "<br>Title: ",
  org$Title,
  "<br><img src='https://picsum.photos/200/300'>"
)
